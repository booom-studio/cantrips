...
describe('createCredentials', () => {
  ...
  it('creates aws user folder if does not exists', async () => {
    ...
    await aws.createCredentials({userFolder: innerPath})
  ...
  it('uses provided values over the environment values', async () => {
    await aws.createCredentials({accessKeyId: 'validAccessParameter', secretAccessKey: 'validSecretAccessParameter', userFolder: tempDir.name})
    var configData = fs.readFileSync(path.join(tempDir.name, 'config'), 'utf8')
